...
void UFunczoneobst::pol2carth(double* theta, double* dist, double carth[][501]){
  ...
  for (int i=0; i<501; i++){
    ...
    snprintf(reply, MRL, "carthX=\"%g\" carthY=\"%g\" />\n",
                  carth[0][i], carth[1][i]);
...
void UFunczoneobst::squareDetect(double theta[501], double dist[501], double poseW[][501], int minRange, double* square, double* lsqlines){
  ...
  if (endIndex != 0){
      ...
      snprintf(reply, MRL, "<cornerIndex=\"%d\" startIndex=\"%d\" dist=\"%g\" endIndex=\"%d\" maxVal=\"%g\" index=\"%d\" />\n",
...
{  // handle a plugin command
  ...
  double r[501];
  double delta;
  double minRange = 5; // min range in meter
  ...
  { // do some action and send a reply
	...
	if (tmp >= 0.020)
      ...
      snprintf(reply, MRL, "<laser l1=\"%g\" l2=\"%g\" l3=\"%g\" \n parkX=\"%g\" parkY=\"%g\" corner angle=\"%g\" Angle line1=\"%g\" Angle line2=\"%g\" length line1=\"%g\" length line2=\"%g\" />\n",
	                  square[0],square[1],square[2],parkingCoordinate[0],parkingCoordinate[1],angleRelation,lsqlines[0],lsqlines[1],lsqlines[2],lsqlines[3]);
